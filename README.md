...
##### Give your java parameters in javacmd.sh file
##### Build the code using "mvn clean compile assembly:single"

### Run Method 1

In Dockerfile Comment the code below and execute the command

```
#ENTRYPOINT [ "/entrypoint.sh" ]

Command
(Replace the path of the local file in the volume)
1. docker run -it -v "D://sample_notes:/opt/sample_notes"  <image_name> 
2. go to /opt and run sh javacmd.sh

Result

A local folder will be created with the parsed data
```

### Run Method 2

Run Dockerfile without Commenting the code below and execute the command
...
Command
(Replace the path of the local file in the volume)
1. docker run -it -v "D://sample_notes:/opt/sample_notes" <Image_name> "bash:/opt/javacmd.sh" 
